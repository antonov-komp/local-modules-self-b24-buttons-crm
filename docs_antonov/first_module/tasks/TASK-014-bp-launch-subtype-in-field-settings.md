...
- [x] В настройках поля `bp_button_field` отображается выбор подтипа: «URL обработчика» и «Запуск бизнес-процесса».
- [x] При выборе «Запуск бизнес-процесса» отображается слайдер или выпадающий список с шаблонами БП для `ENTITY_ID` поля.
- [x] Шаблоны БП фильтруются по типу сущности (смарт-процесс, лид, сделка и т.д.).
- [x] Выбранный шаблон сохраняется в `my_bpbutton_settings.BP_TEMPLATE_ID`.
- [x] При нажатии кнопки в карточке CRM в режиме «Запуск БП» выполняется запуск выбранного шаблона БП для текущего элемента.
- [x] Для сущностей без шаблонов БП выводится сообщение «Нет шаблонов БП для данной сущности».
- [x] Все новые тексты локализованы.
- [x] Нет регрессии: режим «URL обработчика» работает как прежде.
...
## Реализация (что сделано для успешного результата)

### 1. Резолвинг ENTITY_ID в числовой entityTypeId

**Проблема:** `CCrmOwnerType::ResolveID()` не обрабатывает формат UF-сущностей (`CRM_LEAD`, `CRM_DEAL`, `CRM_DYNAMIC_123`), возвращал 0. Из-за этого `getBpStarterConfig` получал некорректный конфиг.

**Решение:**
- В `BpTemplateResolver` добавлен публичный метод `resolveEntityTypeIdFromEntityId()`:
  - `CCrmOwnerType::ResolveIDByUFEntityID()` для `CRM_LEAD`, `CRM_DEAL`, `CRM_CONTACT` и т.д.;
  - отдельная обработка `CRM_DYNAMIC_123` через `TypeTable` (ResolveIDByUFEntityID не поддерживает этот формат).
- `ButtonService::getBpLaunchConfig()` и `CrmAccessChecker::canRead()` переведены на использование `BpTemplateResolver::resolveEntityTypeIdFromEntityId()` вместо `CCrmOwnerType::ResolveID()`.

### 2. Передача ID документа (elementId) в контексте кнопки

**Проблема:** ID элемента (лида, сделки и т.д.) не передавался в data-атрибуты кнопки. CRM передаёт его как `ENTITY_VALUE_ID` в `userField`, а не как `ELEMENT_ID`.

**Решение:** В `ButtonHtmlRenderer::resolveContext()` добавлен приоритетный источник `elementId`:
1. `field['ENTITY_VALUE_ID']` — из userField (карточка CRM);
2. `additional['ENTITY_VALUE_ID']`;
3. `additional['ELEMENT_ID']`, `additional['VALUE']`, `$_REQUEST['ID']`, разбор URL.

### 3. Вывод реальной ошибки в уведомлении

**Проблема:** При ошибке запуска БП показывалось общее сообщение «Ошибка запуска БП», без деталей.

**Решение:** В `button.js` добавлена функция `extractBpError(err)`, извлекающая текст из разных форматов:
- `err.message`, `err.error_description`, `err.text`;
- `err.errors[]` (массив), `err.errors` (объект);
- строка.

Текст реальной ошибки выводится в уведомление (правый верхний угол).

### Затронутые файлы

| Файл | Изменения |
|------|-----------|
| `lib/Service/BpTemplateResolver.php` | Публичный `resolveEntityTypeIdFromEntityId()`, поддержка `CRM_DYNAMIC_*` |
| `lib/Service/ButtonService.php` | Использование `BpTemplateResolver::resolveEntityTypeIdFromEntityId()` |
| `lib/Helper/CrmAccessChecker.php` | Использование `BpTemplateResolver::resolveEntityTypeIdFromEntityId()` |
| `lib/UserField/ButtonHtmlRenderer.php` | Приоритет `ENTITY_VALUE_ID` при определении `elementId` |
| `install/js/my.bpbutton/button.js` | Функция `extractBpError()`, вывод реальной ошибки в уведомление |

---

## Дополнение TASK-014-A: Тумблер скрытия вкладки «Бизнес-процессы»

**Дата создания:** 2026-03-16 (UTC+3, Брест)  
**Статус:** Новая  
**Приоритет:** Средний  
**Исполнитель:** Bitrix24 Программист (коробка, D7 + Vanilla JS)  
**Зависит от:** TASK-014 (реализован)

### Описание

При использовании подтипа «Запуск бизнес-процесса» кнопка `bp_button_field` позволяет запускать БП напрямую из карточки CRM. При этом стандартная вкладка «Бизнес-процессы» в карточке сущности остаётся видимой, что может создавать дублирование и путаницу для пользователей.

**Цель:** добавить **функциональный тумблер** «Скрыть вкладку Бизнес-процессы». При включении — вкладка «Бизнес-процессы» скрывается в карточках CRM для сущностей того типа, к которому привязано поле с данной настройкой.

### Контекст

- Вкладка «Бизнес-процессы» — стандартный элемент интерфейса CRM (лид, сделка, контакт, смарт-процесс). Отображается в карточке сущности.
- Скрытие должно применяться **только** для сущностей, у которых есть поле `bp_button_field` с `ACTION_TYPE = bp_launch` и включённым тумблером `HIDE_BP_TAB = Y`.
- Область действия: по `ENTITY_ID` (тип сущности). Если поле привязано к смарт-процессу 123 — скрывать вкладку во всех карточках элементов этого смарт-процесса.
- Тумблер отображается **только** при `ACTION_TYPE = bp_launch` (при URL-режиме скрытие вкладки БП не имеет смысла).

### Модули и компоненты

| Файл | Изменения |
|------|-----------|
| `local/modules/my.bpbutton/install/index.php` | Миграция: добавить колонку `HIDE_BP_TAB` (char(1), default 'N') в `my_bpbutton_settings` |
| `local/modules/my.bpbutton/lib/Internals/SettingsTable.php` | Добавить поле `HIDE_BP_TAB` в getMap() |
| `local/modules/my.bpbutton/admin/bpbutton_edit.php` | Чекбокс/тумблер «Скрыть вкладку Бизнес-процессы» (виден при bp_launch), сохранение в POST |
| `local/modules/my.bpbutton/lib/Service/SettingsFormService.php` | Валидация и сохранение `HIDE_BP_TAB` |
| `local/modules/my.bpbutton/lib/EventHandler.php` | Обработчик события для скрытия вкладки БП (см. ниже) |
| `local/modules/my.bpbutton/install/js/my.bpbutton/` | JS для скрытия вкладки на клиенте (если потребуется) |
| `local/modules/my.bpbutton/lang/ru/` | Локализация: `HIDE_BP_TAB`, `HIDE_BP_TAB_HINT` |

### Технический подход к скрытию вкладки

**Вариант A (предпочтительный):** событие/хук Bitrix CRM для фильтрации вкладок.

- Исследовать: `crm` — события при построении табов карточки сущности (например, `onCrmEntityDetailsTabs`, `OnBeforeCrmEntityDetailsShow` или аналог).
- Документация: `bitrix/modules/crm/` — компоненты `crm.lead.details`, `crm.deal.details`, `crm.contact.details`, смарт-процессы.
- Логика: в обработчике проверить `ENTITY_ID` текущей карточки; запросить из `my_bpbutton_settings` наличие записи с `ENTITY_ID` = текущий тип, `ACTION_TYPE = bp_launch`, `HIDE_BP_TAB = Y`, `ACTIVE = Y`; если есть — исключить вкладку «Бизнес-процессы» из списка табов.

**Вариант B (запасной):** JS на странице карточки.

- При загрузке карточки CRM: AJAX-запрос или data-атрибут с флагом «скрыть вкладку БП»; если флаг установлен — скрыть вкладку через DOM (найти по data-атрибуту, классу или тексту «Бизнес-процессы») и скрыть/удалить соответствующий элемент.
- Минус: хрупкость при смене вёрстки Bitrix.

**Вариант C:** настройка на уровне типа смарт-процесса (если Bitrix поддерживает).

- Проверить API смарт-процессов: возможность скрыть вкладку БП через конфигурацию типа.

### Ступенчатые подзадачи

1. **Миграция БД:** добавить `HIDE_BP_TAB` в `my_bpbutton_settings`.
2. **SettingsTable:** добавить поле в getMap().
3. **SettingsFormService:** валидация (`Y`/`N`), сохранение.
4. **bpbutton_edit.php:** чекбокс «Скрыть вкладку Бизнес-процессы», видимость только при `ACTION_TYPE = bp_launch`.
5. **Исследование:** найти событие/хук Bitrix для фильтрации вкладок в карточке CRM.
6. **EventHandler:** реализовать скрытие вкладки по событию (вариант A) или подготовить JS (вариант B).
7. **Локализация:** добавить строки в `lang/ru/`.

### Критерии приёмки

- [ ] В форме редактирования кнопки (`bpbutton_edit.php`) при `ACTION_TYPE = bp_launch` отображается тумблер «Скрыть вкладку Бизнес-процессы».
- [ ] Значение тумблера сохраняется в `my_bpbutton_settings.HIDE_BP_TAB`.
- [ ] При `HIDE_BP_TAB = Y` вкладка «Бизнес-процессы» скрыта в карточках CRM для сущностей соответствующего типа.
- [ ] При `HIDE_BP_TAB = N` или при `ACTION_TYPE = url` вкладка отображается как обычно.
- [ ] Работает для лидов, сделок, контактов, компаний и смарт-процессов (если применимо).
- [ ] Локализация выполнена.

### Зависимости

- TASK-014 реализован (подтип bp_launch, запуск БП).
- Коробочная версия Bitrix24.
- Модули `crm`, `bizproc`.

### Граничные случаи

- **Несколько полей bp_button_field** для одной сущности: достаточно одной записи с `HIDE_BP_TAB = Y` — вкладка скрывается.
- **Поле удалено, настройка осталась:** при отсутствии активной записи в `my_bpbutton_settings` для данного `ENTITY_ID` вкладка не скрывается.
- **Права доступа:** скрытие вкладки — визуальное; запуск БП по-прежнему контролируется правами bizproc.

### История правок (дополнение)

- 2026-03-16: Добавлено дополнение TASK-014-A: тумблер скрытия вкладки «Бизнес-процессы».
...
- 2026-03-16: **Реализация завершена.** Добавлен раздел «Реализация (что сделано для успешного результата)»: резолвинг ENTITY_ID через ResolveIDByUFEntityID, передача ENTITY_VALUE_ID как ID документа, вывод реальной ошибки в уведомлении.
- 2026-03-16: Добавлено дополнение **TASK-014-A**: тумблер скрытия вкладки «Бизнес-процессы» (функциональный, сохраняемый в настройках).
